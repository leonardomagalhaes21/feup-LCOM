...
int KBC_counter=0;
//ESPERAR PELA INTERRUPÇÃO DO CODE ESC NUM CICLO
...
//LER O OUTPUT ATÉ QUE O SCANCODE SEJA CODE_ESC
//DAR PRINT AO SCANCODE
//RESTAURAR OS INTERRUPTS DO KEYBOARD
//ESCREVER
int(kbd_test_poll)(){
  while (scancode !=CODE_ESC){
    if (KBC_read_output(0x60,&scancode,0)==0){
      kbd_print_scancode(!(scancode & MAKE_CODE), scancode == IS_TWO_BYTES ? 2 : 1, &scancode);
...
extern int count;
extern int hook_id;
//IGUAL À PRIMEIRA MAS AGORA O CICLO PODE ACABAR QUANDO X SEGUNDOS FOREM PERCORRIDOS SEM NENHUMA INTERRUPÇÃO
int(kbd_test_timed_scan)(uint8_t n){
  int ipc_status;
  uint8_t irq_set,kbd_irq_set;
  message msg;
  uint8_t time;
  time=n;
